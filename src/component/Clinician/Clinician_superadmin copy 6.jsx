...
import { mdiDoctor, mdiStethoscope, mdiChartLine, mdiPlus, mdiDownload } from '@mdi/js';
...
// import { FilterIcon } from 'your-icon-library'; // Ensure to import the FilterIcon
...
const EditClinicianDialog = ({ open, onClose, clinician, onSave }) => {
    const [formData, setFormData] = useState({
        name: clinician?.name || '',
        email: clinician?.email || '',
        mobileNum: clinician?.mobileNum || '',
        specializedIn: clinician?.specializedIn || '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (adminPortal === 'true' && clinician?._id) {
                const url = role === 'Admin'
                    ? `${BaseUrl}/api/admin/doctors/${clinician._id}`
                    : `${BaseUrl}/api/assistant/doctors/${clinician._id}`;
...
// Update the renderActionsCell function in the ClinicianPage component
...
//                 <EditIconButton onClick={() => handleEditClick(clinician)}>
//                     <EditIcon />
//                 </EditIconButton>
//             </Tooltip>
...
const ClinicianPage = () => {
    ...
    const handleEditClick = (clinician) => {
        setSelectedClinicianForEdit(clinician);
        setEditDialogOpen(true);
    };

    const handleEditSave = (updatedClinician) => {
        setClinicians(prevClinicians =>
            prevClinicians.map(c =>
                c._id === updatedClinician._id ? updatedClinician : c
            )
        );
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (modalType === "edit" && selectedClinician) {
                ...
                if (response.data.status === 'success') {
                    // Add the new clinician to the existing list
                    setClinicians(prevClinicians => [...prevClinicians, response.data.body]);
                    toast.success(response.data.message || 'New clinician added successfully!');
                }
            }
            setShowModal(false);
            setSelectedClinician(null); // Reset selected clinician after saving
        } catch (error) {
        ...
        try {
            ...
            if (adminPortal === 'true') {
                const response = await axios.get(
                    `${BaseUrl}/api/patients/get-doctor/${doctorId}`,
                    {
                        headers: {
                            'Authorization': `Bearer ${token}`
                        }
                ...
                {clinician.verified === "no" && (
                    <Tooltip title="Verify Clinician" arrow placement="top">
                        <span>
                            <ActionButton
                                color="view"
                                onClick={() => fetchClinicianDetails(clinician._id)}
                            >
                                Verify
                            </ActionButton>
                        </span>
    ...
    const convertToCSV = (clinicians) => {
        ...
        // Convert clinicians data to CSV rows
        const rows = clinicians.map(clinician => [
            clinician.name,
            clinician.email,
            clinician.mobileNum || 'N/A',
            clinician.specializedIn || 'N/A',
            clinician.experience || 'N/A',
            clinician.location || 'N/A',
            clinician.Active === 'yes' ? 'Active' : 'Inactive',
            clinician.ratings || 'N/A'
        ]);
    ...
    // Inside your ClinicianPage component, add this function
    const handleDownloadCSV = () => {
        const csvData = convertToCSV(getFilteredClinicians());
        const blob = new Blob([csvData], { type: 'text/csv;charset=utf-8;' });
        const link = document.createElement('a');
        const url = URL.createObjectURL(blob);

        link.setAttribute('href', url);
        link.setAttribute('download', 'clinicians_data.csv');
    ...
    const fetchNonVerifiedClinicians = async () => {
        try {
            const token = sessionStorage.getItem('token');
            const response = await axios.get(`${BaseUrl}/api/admin/non-verified-clinicians`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
    ...
    const fetchClinicianDetails = async (clinicianId) => {
        try {
            const token = sessionStorage.getItem('token');
            const response = await axios.get(`${BaseUrl}/api/admin/doctors/${clinicianId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
    ...
    const verifyClinician = async (clinicianId) => {
        ...
        try {
            const token = sessionStorage.getItem('token');
            const response = await axios.put(`${BaseUrl}/api/admin/verify-doctor/${clinicianId}`, {}, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
            ...
            if (response.data.status === 'success') {
                ...
                setDetailsDialogOpen(false); // Close the dialog
    ...
    const DetailsDialog = () => (
        <Dialog open={detailsDialogOpen} onClose={() => setDetailsDialogOpen(false)} PaperProps={{
            sx: {
                borderRadius: '24px',
                background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)',
                color: 'white',
            ...
            <StyledDialogTitle>
                {selectedClinicianDetails ? selectedClinicianDetails.name : 'Clinician Details'}
            </StyledDialogTitle>
            <StyledDialogContent>
                {selectedClinicianDetails && (
                    <Box>
                        <Typography variant="body1">Email: {selectedClinicianDetails.email}</Typography>
                        <Typography variant="body1">Mobile: {selectedClinicianDetails.mobileNum}</Typography>
                        <Typography variant="body1">Specialization: {selectedClinicianDetails.specializedIn}</Typography>
                        <Typography variant="body1">Experience: {selectedClinicianDetails.experience}</Typography>
                        <Typography variant="body1">About: {selectedClinicianDetails.about}</Typography>
                        <Typography variant="body1">Location: {selectedClinicianDetails.location}</Typography>
                        <Typography variant="body1">License Number: {selectedClinicianDetails.licenseNumber}</Typography>
                        <Typography variant="body1">License Expiration: {new Date(selectedClinicianDetails.licenseExpirationDate).toLocaleDateString()}</Typography>
            ...
            <StyledDialogActions>
                ...
                <Button
                    onClick={() => verifyClinician(selectedClinicianDetails._id)}
                    variant="contained"
                ...
                >
                    {loadingVerification ? <CircularProgress size={24} /> : 'Verify'}
                </Button>
            </StyledDialogActions>
        </Dialog>
    );
                                    ...
                                    <TableHeaderBox>
                                        ...
                                        <Box
                                            ...
                                            <StyledFilterButton onClick={handleFilterClick}>
                                                ...
                                                />
                                                {activeFilter === 'all' ? 'All Clinicians' : 'Non-Verified Clinicians'}
                                            </StyledFilterButton>
                                            <StyledFilterMenu
                                                anchorEl={anchorEl}
                                            ...
                                            >
                                                ...
                                                <StyledMenuItem onClick={() => handleFilterSelect('non-verified')} selected={activeFilter === 'non-verified'}>
                                                    Non-Verified Clinicians
                                                </StyledMenuItem>
                                            </StyledFilterMenu>

                                            {/* Add this button before the Add Clinician button */}
                                            <DownloadButton
